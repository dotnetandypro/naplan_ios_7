...
class WordMatchingOptions extends StatefulWidget {
  ...
  @override
  _WordMatchingOptionsState createState() => _WordMatchingOptionsState();
}

class _WordMatchingOptionsState extends State<WordMatchingOptions> {
  String? draggedItem;
  ...
  Widget build(BuildContext context) {
    final stateManager = Provider.of<WordMatchingStateManager>(context);
    stateManager.initialize(widget.questionId, widget.options);
  ...
  Widget _buildOptionItem(BuildContext context,
      WordMatchingStateManager stateManager, String option, int index) {
    final state = stateManager.getQuestionState(widget.questionId);
    ...
    return Padding(
      ...
      child: Draggable<String>(
        data: option,
        feedback: _buildDragItem(option, isImage, itemWidth),
        ...
        child: GestureDetector(
          onTap: () {
            if (isMatched) {
              stateManager.removeMatch(widget.questionId, option);
              widget.onAnswerSelected(state.selectedAnswers);
            }
          },
          child: Container(
            width: itemWidth,
  ...
  Widget _buildCategoryItem(BuildContext context,
      WordMatchingStateManager stateManager, String category, int index) {
    final state = stateManager.getQuestionState(widget.questionId);
    ...
    return Padding(
      ...
      child: DragTarget<String>(
        ...
        onAcceptWithDetails: (details) {
          ...
          // If this category already has a match, remove it first
          String? existingMatch = state.userMatches.entries
              .firstWhere(
          ...
          if (existingMatch.isNotEmpty) {
            stateManager.removeMatch(widget.questionId, existingMatch);
          }
